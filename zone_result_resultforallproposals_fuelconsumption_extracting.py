...
def parse_zone_result_resultforallproposals_fuelconsumption(etree, logging, serial_id, content, esi, cur, conn):

    result_resultforallproposals_fuelconsumption_dict = {}
    # result_resultforallproposals_fuelconsumption_dict['FuelConsumption'] = {'Fuel': {'Material': None, 'Unit': None, 'EnergyPerUnit' : None, 'CO2PerUnit': None}, 
    # 'FuelPrice': {'CostPerUnit': None, 'FixedCostPerYear': None}, 'FuelConsumption': None, 'IsForHeating' : None}
    result_resultforallproposals_fuelconsumption_dict['Fuel'] = {'Material': None, 'Unit': None, 'EnergyPerUnit' : None, 'CO2PerUnit': None} 
    result_resultforallproposals_fuelconsumption_dict['FuelPrice'] = {'CostPerUnit': None, 'FixedCostPerYear': None} 
    result_resultforallproposals_fuelconsumption_dict['FuelConsumption'] = None 
    result_resultforallproposals_fuelconsumption_dict['IsForHeating'] = None 
    result_resultforallproposals_fuelconsumption_dict['zone_result_resultforallproposals_id_ref'] = serial_id 

    dict_local = generic_extracting(etree, result_resultforallproposals_fuelconsumption_dict)
    generate_sql_insert("resultforallproposals_fuelconsumption", dict_local, content, logging, esi, cur, conn, 'id')
...
#                         if key1 == 'Status':
#                             print('Now Im here 5')
#                             parse_zone_result_status_fuelconsumption(et, esi, serial_id, logging, cur, conn)
...
#                     if content == "ZoneResult":
#                         parse_zone_result_status_fuelconsumption(etree, content, logging, esi, cur, conn)
...
# def handle_dicts(pretty_doc, content, esi, serial_id, logging, cur, conn):
#     for key, vals in content.items():
#         etree = get_root(pretty_doc, key)
#         for et in etree:
#             for val in vals:
#                 etree_val = et.findall('.//{*}' + val)
#                 for et_val in etree_val:
#                     if key == 'ZoneResult' and val == 'Status':
#                         parse_zone_result_status_fuelconsumption(et_val, content, logging, esi, cur, conn)
...
#     #(FB says): It seemed like making Label under InputData into a dictionary was a good idea
#     # because it had to go a few levels down and ProposalGroups will appear in another tag's hierarchy.
#     content_types = [{'ZoneResult': {'Status': None}}]  #[{'InputData':[{"Label": ["ProposalGroups"]}]}, {'ResultData': ["LabelResults", "BuildingResult"]}, 'Building']
#     #doc = "/home/energy_extract/test_mape/XML_docs/copy_pretty_100115068.xml"
#     esi = None
#     serial_id = None
#     with get_connection() as conn:
